...
class CsWriter {
  ...
  public:
    CsWriter(const enc::encoded *tseq, const enc::encoded *qseq,
             const mm_extra_t *x)
        : tseq(tseq)
        , qseq(qseq)
        , x(x) {}
  ...
  private:
    inline void on_match() { n_stored_matches += 1; }
    ...
    inline void process_intron(size_t len) {
        assert(len >= 2);
        cs << '~' << enc::decode(tseq[0]) << enc::decode(tseq[1]) << len
           << enc::decode(tseq[len - 2]) << enc::decode(tseq[len - 1]);
        tseq += len;
        levenshtein += len;
    }
};
