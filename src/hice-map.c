...
int main(int argc, char *argv[])
{
	...
	// Create pipes
	pipe_t* raw = pipe_new(sizeof(hc_read_pair_t), 100);
	pipe_t* mapped = pipe_new(sizeof(hc_mapped_pair_t), 100);
	
	// Start a thread reading raw pairs from argv[2] and argv[3]
	hc_rthread_t* rthread  = hc_rthread_create(&argv[2], raw);
	if (!rthread) return 3;

	// Start aligner threads
	hc_athread_t* athreads[2];
	for (int i=0; i<NWORKERS; ++i) {
		athreads[i] = hc_athread_create(mi, &mopt, raw, mapped);
	}
	...
	hc_wthread_t* wthread = hc_wthread_create(mapped);
	pipe_free(mapped);
